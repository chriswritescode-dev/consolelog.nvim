...
describe("Explain prompt", function()
  it("builds numbered rows with absolute line numbers", function()
    local result = prompt.build({ "const a = 1", "return a" }, 12, 12, 13, "javascript")
  ...
  it("bounds the explained range while the rest stays context", function()
    local result = prompt.build({ "import x", "const a = 1", "return a" }, 11, 12, 12, "javascript")
    assert.is_true(result:find("11: import x", 1, true) ~= nil, "context lines are numbered too")
    assert.is_true(result:find("Explain only lines 12-12", 1, true) ~= nil, "instruction bounds the explained range")
    assert.is_true(result:find("context", 1, true) ~= nil, "instruction marks the rest as context")
  end)

  it("names the filetype and uses the response key constant", function()
    local result = prompt.build({ "const a = 1" }, 12, 12, 12, "javascript")
  ...
  it("returns nil for an empty source", function()
    assert.is_nil(prompt.build({}, 1, 1, 1, "lua"))
  end)
end)
...
describe("Explain response parsing", function()
  ...
  it("returns nil plus an error message for unusable responses", function()
    ...
    assert.not_nil(annotations_empty, "an explicit empty list is a valid answer")
    assert.equals(0, #annotations_empty)
    assert.is_nil(err_empty)
